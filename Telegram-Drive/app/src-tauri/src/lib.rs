...
pub fn run() {
    ...
    // Prefer fixed token from environment for production; fall back to random token for local/dev
    let stream_token = std::env::var("STREAM_TOKEN").unwrap_or_else(|_| generate_stream_token());
